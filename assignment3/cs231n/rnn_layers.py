...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros([n, t, h])
    for i in range(t):
        pre_h, cache_values = rnn_step_forward(x[:, i, :], pre_h, Wx, Wh, b)
        h[:, i, :] = pre_h
        cache.append(cache_values)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(t)):
        ...
        dx[:, i, :], dprev_h, dWxt, dWht, dbt = rnn_step_backward(dnext_h, cache[i])
        dWx += dWxt
        dWh += dWht
        db += dbt
    dh0 =  dprev_h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dout = o * dnext_h
    dstate = dout * (1 - np.square(np.tanh(next_c))) + dnext_c
    
    dprev_c = dstate * f
    
    dg = dstate * i * (1 - np.square(g))
    di = dstate * g * i * (1 - i)
    df = dstate * prev_c * f * (1 - f)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(t):
        pre_h, prev_c, cache_values = lstm_step_forward(x[:, i, :], pre_h, prev_c, Wx, Wh, b)
        h[:, i, :] = pre_h
        cache.append(cache_values)
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(t)):
        dnext_h = dprev_h + dh[:,i,:]
        dx[:, i, :], dprev_h, dprev_c, dWxt, dWht, dbt = lstm_step_backward(dnext_h, dprev_c, cache[i])
        dWx += dWxt
        dWh += dWht
        db += dbt
    dh0 =  dprev_h
